...
def configure(
    *,
    ...
    if enable_watermark is not None:
        _opts.enable_watermark = enable_watermark
...
def _patched_load(path):
    ...
    if isinstance(ret, tuple) and len(ret) == 4:
        state, cfg, train_cfg, text_enc_cfg = ret
        has_four = True
    else:
        state, cfg, train_cfg = ret
        text_enc_cfg = None
        has_four = False
    quant_layers = None
    extra_layers = None
    ...
    if not quant_layers and not extra_layers and not embed_layers:
        return (state, cfg, train_cfg, text_enc_cfg) if has_four else (state, cfg, train_cfg)
...
def _patched_from_key(cls, key):
    ...
    ret = inference_runtime._load_checkpoint_for_inference(
        Path(key.checkpoint)
    )
    if isinstance(ret, tuple) and len(ret) == 4:
        model_state, model_cfg_dict, train_cfg, text_encoder_config = ret
    else:
        model_state, model_cfg_dict, train_cfg = ret
    ...
    text_tokenizer_source, text_tokenizer_is_local = inference_runtime._resolve_tokenizer_source(
        Path(key.checkpoint),
    ...
    if model_cfg.use_caption_condition:
        caption_tokenizer_source, caption_tokenizer_is_local = inference_runtime._resolve_tokenizer_source(
            Path(key.checkpoint),
    ...
    default_max_ref_seconds = inference_runtime._default_max_ref_seconds(train_cfg)
...
def patch() -> None:
    ...
    if not _opts.enable_watermark:
        try:
            from irodori_tts.watermark import SilentCipherWatermarker
            def _noop_init(self, *args, **kwargs):
                self.model = None
            SilentCipherWatermarker.__init__ = _noop_init
            print("[irodori_tts_lite] patched SilentCipherWatermarker (watermark disabled)")
        except ImportError:
            pass
