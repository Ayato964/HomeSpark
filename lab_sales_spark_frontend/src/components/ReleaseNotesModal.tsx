        ...
        <div style={{
          ...
          <div>
            ...
            <ul style={{ margin: 0, paddingLeft: '20px', display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '13px', color: 'var(--text2)', lineHeight: 1.6 }}>
              ...
              <li><strong>🛡️ 会話識別サブエージェント & `is_conv` 誤爆防止ステートマシン</strong>: 周囲の雑音・独り言・他人との会話をAIが勝手に拾って誤作動するのを防止。「GeMo」「ジェモ」等のウェイクワードや明確な指示がある時のみ対話を開始し、対話中（`is_conv = true`）は遅延ゼロでリアルタイム対話。用件完了時または20秒無音で自動待機復帰します。</li>
              <li><strong>📧 汎用 IMAP / SMTP 外部メール連携 (`list_external_emails`, `send_external_email`)</strong>: 会社の独自ドメインメールや、さくら、エックスサーバー、Yahoo!、Outlook などのメールアカウントを複数追加連携可能に。ワンクリック・プリセット設定と安全な接続テスト機能を備え、GeMoが外部メールの確認・検索・送信を行えるようになりました。</li>
