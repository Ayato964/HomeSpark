...
export class ChatService {
  ...
  public static readonly BASE_SYSTEM_RULES: string =
    ...
    "デジタル名刺・顧客・人物プロファイルツール（get_digital_business_cards, search_digital_business_cards, create_digital_business_card, delete_digital_business_card）を利用できます。名刺や顧客情報の照会、新規登録、編集、検索などを依頼された場合は積極的にこれらのツールを活用してください。\n\n" +
  ...
  public static readonly CHAT_SYSTEM_PROMPT: string =
    ...
    "- ユーザーのLPデザインの提案, API設計, 週次レポート作成, 競合分析などをサポートします。必要に応じてファイル作成ツールを実行し、構成案をマークダウン形式で保存してください。";
  ...
  public static readonly VOICE_SYSTEM_PROMPT: string =
    ...
    "AI: 😊明日の東京は最高33度の曇りで、午後は雨が降る可能性があるため傘があると安心ですよ！";
